...
```
 val all_methods_spawning_threads = cpg.call.code(".*Thread.*").map(_.methodFullName).l.filterNot(_.startsWith("System"))
```
 #### result>
```
all_methods_spawning_threads: List[String] = List(
...
```
val process_fnv_services = services_blacklist.map { i => 
    val seed = 6605813339339102567L
    val op = FNV.hash64a(i.getBytes) ^ seed
	op 
}
